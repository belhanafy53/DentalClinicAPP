...
namespace DentalClinicAPP.Forms.BasicsCode
{
    public partial class UnitesTypeFrm : Form
    {
        ...
        private void UnitesTypeFrm_Load(object sender, EventArgs e)
        {
            RowIDtxt.Text = "";
        ...
        private void simpleButton3_Click(object sender, EventArgs e)
        {
            ...
            this.Controls.Clear();
            InitializeComponent();
            var listAnalysis = DNtMDB.Tbl_UniteType.OrderBy(x => x.Code).ToList();
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            if (CodeTxt.Text == "")
            {
                ...
                {
                    MessageBox.Show("من فضلك ادخل كود الوحده المراد اضافته !");
                }
            ...
            else
            {
                if (RowIDtxt.Text == "")
                {
                    ...
                    Tbl_UniteType Act = new Tbl_UniteType
                    {
                        Code = CodeTxt.Text,
                    ...
                    DNtMDB.Tbl_UniteType.Add(Act);
                    DNtMDB.SaveChanges();
                    //int Vint_LastRow = Act.;
                    //---------------خفظ ااحداث 
                    SecurityEvent sev = new SecurityEvent
                    {
                        ActionName = "Unite Add",
                        TableName = "Tbl_UniteType",
                ...
                else if (RowIDtxt.Text != "")
                {
                    ...
                    var result = DNtMDB.Tbl_UniteType.FirstOrDefault(x => x.Code == CodeTxt.Text);
                    if (result != null)
                    {
        ...
        {//var validationSaveUser = Program.SecurityProceduresList.FirstOrDefault(w => w.FormId == 4 && w.ProcdureId == 4);
            ...
            if (Vint_D1rows != 0 && DescEtxt.Text != "")
            {
                ...
                if (LangTxtBox.Text == "en")
                {
                    var result1 = MessageBox.Show("Do you want to delete this Unite Type   ?", "Delete Unite Type ", MessageBoxButtons.YesNo);
                    if (result1 == DialogResult.Yes)
                    {


                        var resultR = DNtMDB.Tbl_UniteType.FirstOrDefault(x=>x.Code == Vst_Code);
                        DNtMDB.Tbl_UniteType.Remove(resultR);
                        DNtMDB.SaveChanges();
                        //---------------خفظ ااحداث 
                        SecurityEvent sev = new SecurityEvent
                        {
                ...
                {
                    var result1 = MessageBox.Show("هل تريد حذف هذه الوحده  ؟", "حذف الوحده ", MessageBoxButtons.YesNo);
                    if (result1 == DialogResult.Yes)
                    {

                        var resultR = DNtMDB.Tbl_UniteType.Find(Vst_Code);
                        DNtMDB.Tbl_UniteType.Remove(resultR);
                        DNtMDB.SaveChanges();
                        //---------------خفظ ااحداث 
                        SecurityEvent sev = new SecurityEvent
            ...
            else
            {
                ...
                {
                    MessageBox.Show("من فضلك اختر الوحده المراد حذفها");
                }
